...
export class ShopScene extends Phaser.Scene {
  ...
  render() {
    ...
    pageCards.forEach((def, idx) => {
      ...
      cv.setPosition(x, y);
      cv.setInteractive();
      attachCardTooltip(this, cv, def);
      this.root.add(cv);

      const labelY = y + Math.round(70 * CARD_SCALE);
      const owned = Boolean(p.collection[def.id]);
      if (owned) {
        this.text(x - 18, labelY, t('shop.owned'), '#9a8a6a', '15px');
      } else if (canBuy(p, def.id)) {
        const buy = this.text(x - 38, labelY, t('shop.buy', { n: def.cost }), '#9fe3d0', '15px')
          .setInteractive({ useHandCursor: true });
        buy.on('pointerdown', () => {
          buyCard(p, def.id);
          persist();
        ...
        this.text(x - 42, labelY, t('shop.poor', { n: def.cost }), '#ff9d9d', '14px');
      }
    });
